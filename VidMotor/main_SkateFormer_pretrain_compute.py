...
# SkateFormer receives the 16-point partition produced from the same 17-joint skeleton sequence.
STANDARD_INPUT_SHAPE = (1, 3, 288, 16, 1)
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        self.save_arg()
        self.load_model()
        self.complexity = self.report_params()
        self.report_flops()
        self.load_optimizer()
    ...
    def report_params(self):
        ...
        result = {'total_params': int(total), 'trainable_params': int(trainable)}
        self.print_log(f'Model Params: total={total:,}, trainable={trainable:,}.')
        self.save_complexity(result)
        return result

    def save_complexity(self, result=None):
        complexity = result if result is not None else self.complexity
        with open(f'{self.arg.work_dir}/model_complexity.yaml', 'w') as f:
            yaml.safe_dump(complexity, f, sort_keys=False)

    def report_flops(self):
        """Estimate forward FLOPs with a fixed standardized input."""
        model = self.raw_model()
        sample = torch.randn(*STANDARD_INPUT_SHAPE, dtype=torch.float32)
        sample = Variable(sample.float().cuda(self.output_device), requires_grad=False)
        index_t = torch.linspace(-1, 1, STANDARD_INPUT_SHAPE[2], device=sample.device).unsqueeze(0)
        ...
        try:
            ...
            with torch.no_grad(), torch.profiler.profile(activities=activities, with_flops=True) as prof:
                model(sample, index_t)
            flops = sum(float(event.flops or 0) for event in prof.key_averages())
            self.complexity.update({
                'input_shape': [int(v) for v in sample.shape],
                'forward_flops_per_sample': float(flops),
                'flops_note': ('Forward FLOPs for the fixed standardized input; '
                               'unsupported custom operators may be omitted by PyTorch profiler.')
            })
            self.print_log(f'Model FLOPs: {flops:,.0f} per sample; input shape={tuple(sample.shape)}.')
        except Exception as exc:
            self.complexity['flops_error'] = f'{type(exc).__name__}: {exc}'
            self.print_log(f'FLOPs profiling failed: {self.complexity["flops_error"]}')
        finally:
            model.train(was_training)
            self.save_complexity()

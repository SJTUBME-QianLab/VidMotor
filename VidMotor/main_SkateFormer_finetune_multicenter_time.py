...
class Processor():
    ...
    def eval(self):
        ...
        for loader_index in range(4):
            with torch.no_grad():
                for batch_idx, (data, _, index_t, index) in enumerate(self.data_loaders['evaluate'][loader_index]):
                    data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                    index_t = Variable(index_t.float().cuda(self.output_device), requires_grad=False)
            ...
            avg_time = start_event.elapsed_time(end_event) / (len(self.data_loaders['evaluate'][loader_index].dataset) * 10)
    ...
    def start(self):
        ...
        # Load and evaluate across multiple centers

...
def get_parser():
    ...
    parser.add_argument('--save-interval', type=int, default=1, help='interval (in epochs) for saving models')
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        self.print_log(f'\tMean training loss: main {np.mean(loss_value):.4f}; nc: {np.mean(loss_nc_value):.4f}.',
                       self.save_path)
    ...
    def eval(self, save_score=False, loader_indices=None, epoch=None):
        """Evaluate model performance on test data."""
        self.model.eval()
        accuracies = []
        evaluate_loaders = self.data_loaders['evaluate'] if loader_indices is None else [
            self.data_loaders['evaluate'][i] for i in loader_indices
        ]
        with torch.no_grad():
            for evaluate_loader in evaluate_loaders:
                with tqdm(evaluate_loader, ncols=100) as t:
                    score_frag = []
                    ...
                    accuracy = evaluate_loader.dataset.top_k(output_score, 1)
                    accuracies.append(accuracy)
                    self.print_log(
                        f'\tEvaluate Dataset: {evaluate_loader.dataset.dataset_name}; Accuracy: {accuracy * 100:.2f}',
                        self.save_path)

                    # Optionally save prediction scores
                    if save_score:
                        out_dir = os.path.join(self.save_path, evaluate_loader.dataset.dataset_name)
                        os.makedirs(out_dir, exist_ok=True)
                        with open(f'{out_dir}/epoch{epoch}_score.pkl', 'wb') as f:
                            pickle.dump(dict(zip(evaluate_loader.dataset.sample_name, output_score)), f)
    ...
    def start(self):
        ...
        self.data_loaders = {"train": [], "evaluate": []}
        ...
        validate_set_center1 = Feeder(self.arg.test_feeder_args[0], self.arg.data_centered, self.arg.data_dim,
        ...
        test_set_center4 = Feeder(self.arg.test_feeder_args[3], self.arg.data_centered, self.arg.data_dim,
                                  self.arg.data_norm, self.arg.score_norm, self.arg.label_type,
                                  data_length=train_set.data_length)
        self.data_loaders['evaluate'].append(
            DataLoader(dataset=validate_set_center1, batch_size=self.arg.test_batch_size, shuffle=False,
        ...
        for epoch in range(0, self.arg.num_epoch):
            ...
            val_accuracy = self.eval(save_score=self.arg.save_score, loader_indices=[0], epoch=epoch + 1)
            self.print_log(f'\tValidation center1 accuracy: {val_accuracy * 100:.2f}', self.save_path)
            if val_accuracy > best_val_accuracy:
                best_val_accuracy = val_accuracy
                best_epoch = epoch + 1
        self.print_log(f'work_dir:{self.save_path}', self.save_path)
        self.model.load_state_dict(torch.load(f'{self.save_path}/epoch-{best_epoch}.pt', map_location='cpu'))
        self.print_log(
            f'Loaded best center1 checkpoint from epoch {best_epoch}; '
            f'validation accuracy: {best_val_accuracy * 100:.2f}',
            self.save_path,
        )
        self.eval(save_score=self.arg.save_score, loader_indices=[1, 2, 3], epoch=best_epoch)

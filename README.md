...
**A video-based motor function foundation model for widely-available medical assessment**
...
- [Recommended Workflow for Task-specific Fine-tuning](#recommended-workflow-for-task-specific-fine-tuning)
...
# Recommended Workflow for Task-specific Fine-tuning

The repository includes [`example_script.py`](example_script.py), an example for downstream fine-tuning with the default VidMotor backbone. It uses the `SPHERE-Stair-Gait` task as a concrete example and is intended to be adapted to a user's own dataset quickly.
...
2. Loads and preprocesses a downstream dataset.
3. Performs a subject-wise data split.
4. Conducts a grid search over six recommended SGD configurations:
   - Batch sizes: `4`, `8`
   - Learning rates: `0.001`, `0.0005`, `0.0001`
5. Selects the best model.
6. Evaluates the selected model using `Accuracy`, `Balanced Accuracy`, `F1 Score`, and `AUROC`.


## Before running

Install the dependencies described in `environment.yaml`.
The pretrained checkpoint is `./VidMotor/pretrain-ST-GCN.pt`.
The default data file is `./data/Example/data_and_label_SPHERE-Stair-Gait.pkl`.

The feeder expects a pickle dictionary whose keys are sample names and whose values contain at least:
```python
{
    "data": <skeleton sequence>,
    "class_label": <integer class label>
}
```

Labels should be integer class IDs from `0` to `num_class - 1`. The example expects sample names beginning with a subject ID followed by an underscore, for example `subject01_trial01_...`; this is how it prevents samples from the same subject entering both splits.

## Run the example

```bash
python example_script.py --device 0
```
### Common checks when adapting:

- `Data file not found`: verify `DATASET_CONFIG['data_path']` and run the command from the repository root.
- Unexpected class errors: check that labels are zero-based integers and match `num_class`.

## Outputs

All outputs are written to `./example_results` using the configured dataset name:

```text
<dataset>_grid_search.csv                  # All grid trials and validation metrics
<dataset>_best_model.pt                    # Selected model checkpoint
<dataset>_best_model_results.json     # Selected hyperparameters and final metrics
```
...
- Directory structure of `./data`:
  ...
  │   ├── <dataset_name>.py
  │   └── ...
  ├── Example           # Example dataset (recommended workflow for task-specific fine-tuning)
  │   └── data_and_label_<dataset_name>.pkl
...
- Example output:
  ...
  ├── config.yaml                            # Configuration file
  ├── log.txt                                # Per-sample inference time log
  └── epoch-<epoch_number>.pt                # Fine-tuned model checkpoint
  ```
...
https://figshare.com/articles/dataset/VidMotor_data_and_model_resources/33965521
...
|    [CTR-GCN](https://github.com/Uason-Chen/CTR-GCN)        |   [pretrain-CTR-GCN.pt](https://figshare.com/articles/dataset/VidMotor_data_and_model_resources/33965521)    |      main_CTR-GCN_pretrain.py      |       config/Pretrain_CTR-GCN/       |       main_CTR-GCN_finetune.py,<br/>main_CTR-GCN_finetune_multicenter.py       |   config/Finetune_CTR-GCN/    |
| [PoseFormerV2](https://github.com/QitaoZhao/PoseFormerV2)  | [pretrain-PoseFormerV2.pt](https://figshare.com/articles/dataset/VidMotor_data_and_model_resources/33965521) |   main_PoseFormerV2_pretrain.py    | config/Pretrain_PoseFormerV2/ |  main_PoseFormerV2_finetune.py,<br/>main_PoseFormerV2_finetune_multicenter.py  | config/Finetune_PoseFormerV2/ |
| [SkateFormer](https://github.com/KAIST-VICLab/SkateFormer) | [pretrain-SkateFormer.pt](https://figshare.com/articles/dataset/VidMotor_data_and_model_resources/33965521)  |    main_SkateFormer_pretrain.py    |   config/Pretrain_SkateFormer/    |   main_SkateFormer_finetune.py,<br/>main_SkateFormer_finetune_multicenter.py   | config/Finetune_SkateFormer/  |
|   [DSTformer](https://github.com/Walter0807/MotionBERT)    |  [pretrain-DSTformer.pt](https://figshare.com/articles/dataset/VidMotor_data_and_model_resources/33965521)   |         main_DSTformer_pretrain.py |     config/Pretrain_DSTformer/      |     main_DSTformer_finetune.py,<br/>main_DSTformer_finetune_multicenter.py     |  config/Finetune_DSTformer/   |
...
|                    Comparative approach                    | Script directory |                             Run scripts                              |
|:----------------------------------------------------------:|:----------------:|:--------------------------------------------------------------------:|
...
To reproduce Figures 2–6 and Extended Data Figure 4 in our paper, run:

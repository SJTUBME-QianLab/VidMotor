    ...
    def eval(self, epoch):
        ...
        with torch.no_grad():
            for evaluate_loader in self.data_loaders['evaluate']:
                score_frag = []
                for batch_idx, (data, _, index_t, index) in enumerate(evaluate_loader):
                    data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                    index_t = Variable(index_t.float().cuda(self.output_device), requires_grad=False)
                ...
                if epoch == self.arg.num_epoch - 1:
                    ...
                    writer.write_result(
                        dataset=evaluate_loader.dataset.dataset_name, model_name='VidMotor_SkateFormer', accuracy=final_accuracy * 100, f1=final_f1 * 100,
    ...
    def start(self):
        ...
        for test_dataset_config in arg.test_feeder_args:
            ...
            validate_set = Feeder(test_dataset_config, self.arg.data_centered, self.arg.data_dim,
                              ...
                              data_length=train_set.data_length, data_indices=validate_sample_name, stage='test',

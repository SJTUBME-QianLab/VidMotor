...
class Processor():
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for batch_idx, (data, _, index) in enumerate(self.data_loaders['evaluate'][0]):
        ...
        avg_time = start_event.elapsed_time(end_event) / (len(self.data_loaders['evaluate'][0].dataset) * 10)
        print(f'\t\t\tCurrent repetition: inference time {avg_time:.4f} milliseconds')
        return avg_time

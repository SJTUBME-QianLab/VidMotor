...
class Processor():
    ...
    def eval(self, epoch):
        ...
        self.model.eval()
        with torch.no_grad():
            for evaluate_loader in self.data_loaders['evaluate']:
                score_frag = []
                for batch_idx, (data, _, index) in enumerate(evaluate_loader):
                    data = Variable(data.float().cuda(self.output_device), requires_grad=False)
                    bs, nc = data.shape[:2]
                ...
                true_label = np.array(evaluate_loader.dataset.ori_label)
                ...
                if epoch == self.arg.num_epoch - 1:
                    ...
                    writer.write_result(
                        dataset=evaluate_loader.dataset.dataset_name, model_name='SkeleT-GCN', accuracy=final_accuracy * 100,
    ...
    def start(self):
        ...
        for test_dataset_config in arg.test_feeder_args:
            ...
            train_subjects, validate_subjects = dataset_subjects[:num_train], dataset_subjects[num_train:]
            ...
            if 'PD4T' in dataset.dataset_name:
                ...
                validate_sample_name = [s for s in sample_name if any('_' + sub in s for sub in validate_subjects)]
            else:
                train_sample_name = [s for s in sample_name if any(sub + '_' in s for sub in train_subjects)]
                validate_sample_name = [s for s in sample_name if any(sub + '_' in s for sub in validate_subjects)]
            assert len(train_sample_name) + len(validate_sample_name) == len(sample_name)
            ...
            train_set = Feeder(test_dataset_config, data_centered=self.arg.data_centered, data_dim=self.arg.data_dim,
                               ...
                               num_clips=1)
            validate_set = Feeder(test_dataset_config, data_centered=self.arg.data_centered, data_dim=self.arg.data_dim,
                              data_norm=self.arg.data_norm, score_norm=self.arg.score_norm,
                              label_type=self.arg.label_type, data_indices=validate_sample_name, stage='test',
                              num_clips=10)
            ...
            if os.path.exists(os.path.join(
                    os.path.join(self.arg.save_path.split('.xlsx')[0], validate_set.dataset_name,
            ...
            self.data_loaders['evaluate'].append(
                DataLoader(dataset=validate_set, batch_size=self.arg.test_batch_size, shuffle=False,
            ...
            self.data_loaders['evaluate'].clear()

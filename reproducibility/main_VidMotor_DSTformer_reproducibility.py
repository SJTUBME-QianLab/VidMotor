...
class Processor():
        ...
        with torch.no_grad():
            for evaluate_loader in self.data_loaders['evaluate']:
                ...
                if epoch == self.arg.num_epoch - 1:
                    ...
                    writer.write_result(
                        dataset=evaluate_loader.dataset.dataset_name, model_name='VidMotor_DSTformer', accuracy=final_accuracy * 100, f1=final_f1 * 100, auc=final_auc * 100, balanced_acc=final_balanced_acc * 100)
                    row = (
    ...
    def start(self):
        ...
        for test_dataset_config in arg.test_feeder_args:
            ...
            validate_set = Feeder(test_dataset_config, self.arg.data_centered, self.arg.data_dim,
                              self.arg.data_norm, self.arg.score_norm, self.arg.label_type,
                              data_length=train_set.data_length,
                              data_indices=validate_sample_name, scale_range=self.arg.scale_range_test, is_train=False)

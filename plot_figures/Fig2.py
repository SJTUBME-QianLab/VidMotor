...
xlabel_names_fig2 = {
    "SPHERE-Stair-Gait": "SPHERE-Stair-Gait\n(5-class)",
    "SPHERE-Surface-Gait": "SPHERE-Surface-Gait\n(3-class)",
    "SPHERE-Sit": "SPHERE-Sit\n(binary)",
    "SPHERE-Stand": "SPHERE-Stand\n(binary)",
...
def plot_bar(datasets, data_dict, model_ordered, figsize, ylabel, save_path=None):
    ...
    plt.plot(
        [xmin, xmax],
        [compara_mean_scatter, compara_mean_scatter],
        color="#8A8A8A",
        linestyle="--",
        linewidth=0.8,
        alpha=0.8,
    ...
    if not np.isnan(pval):
        ...
        ax.plot(
            [x_right, x_right],
            [ours_mean, compara_mean_scatter],
        ...
        if pval < 0.0001:
            ax.text(
                x_right + 0.01,
                (ours_mean + compara_mean_scatter) / 2,
                rf"$P$ < 0.0001",
                transform=ax.get_yaxis_transform(),
                va="center",
                ha="left",
    ...
    plt.xticks(
        x + (n_models - 1) * bar_width / 2,
        [xlabel_names_fig2[d] for d in datasets],
        ha="center",
        rotation=30,
        fontproperties=arial_font,
    )

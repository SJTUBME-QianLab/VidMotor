...
xlabel_names_fig3 = {
    "SSBD-Line-Gait": "SSBD-Line-Gait\n(binary)",
    "EHE-Hands-Wave": "EHE-Hands-Wave\n(binary)",
    "EHE-Hands-UpDown": "EHE-Hands-UpDown\n(binary)",
    "EHE-Waist-BendL": "EHE-Waist-BendL\n(binary)",
    "EHE-Waist-BendR": "EHE-Waist-BendR\n(binary)",
    "EHE-Forward-Gait": "EHE-Forward-Gait\n(binary)",
    "EHE-Backward-Gait": "EHE-Backward-Gait\n(binary)",
    "PD-Round-Gait": "PD-Round-Gait\n(binary)",
    "PD-Walkway-Gait": "PD-Walkway-Gait\n(3-class)",
    "PD4T-Round-Gait": "PD4T-Round-Gait\n(4-class)",
    "PD4T-Leg-Agility": "PD4T-Leg-Agility\n(4-class)",
}
dataset_fig3 = list(xlabel_names_fig3.keys())
xlabel_names_fig4 = {
    "IRDS-Elbow-FlexionL": "IRDS-Elbow-FlexionL\n(binary)",
...
def plot_scatter_subaxes(
    datasets, data_dict, model_ordered, figsize, ylabel, xlabel_dict, savepath=None
):
    model_values = np.array(
        [[data_dict[model][ds]["mean"] for ds in datasets] for model in model_ordered]
    )
    ...
    for i, ds in enumerate(datasets):
        ...
        if i == len(datasets) - 1:
            ...
            for j, model in enumerate(model_ordered):
                ...
                ax.scatter(
                    scatter_xcor,
                    y,
                    facecolors=color_models[model],
                    edgecolors="none",
                    marker=marker_models[model],
                    s=scatter_size,
                    alpha=0.75,
                    clip_on=False,
                )
        ...
        if ylabel == "AUROC":
            ...
            ax.set_yticklabels(
                [f"{y_lim_bottom:.1f}", f"{y_lim_top:.1f}"], fontproperties=arial_font
            )
        else:
            y_min = min(y_values) - 0.5
            y_lim_bottom = np.floor(y_min)
            if (
                ("F1" in ylabel and ds == "EHE-Forward-Gait")
                or ("Balanced" in ylabel and ds == "IRDS-Elbow-FlexionR")
                or ("Balanced" in ylabel and ds == "IRDS-Shoulder-AbductionL")
                or ("F1" in ylabel and "IRDS-Elbow-FlexionL" in ds)
                or ("F1" in ylabel and "IRDS-Side-Tap" in ds)
                or ("F1" in ylabel and ds == "PD-Round-Gait")
                or ("F1" in ylabel and ds == "Mean" and len(datasets) == 11)
                or ("Balanced" in ylabel and ds == "IRDS-Side-TapL")
            ):
                y_lim_bottom -= 1
            ...
            ax.set_yticks([y_lim_bottom, y_lim_top])
            ax.set_yticklabels(
                [f"{int(y_lim_bottom)}", f"{int(np.ceil(y_lim_top))}"],
    ...
    if not np.isnan(pval):
        ...
        if pval < 0.0001:
            ax.text(
                x_right + 0.1,
                (our_mean + compara_mean_scatter) / 2,
                r"$P$ < 0.0001",
                transform=ax.get_yaxis_transform(),
                va="center",
                ha="left",
...
def plot_legend(save_path):
    handles = [
        Line2D(
            [0],
            [0],
            marker=marker_models[lab],
            color="w",
            markerfacecolor=color_models[lab],
            markersize=8,
            label=lab,
        )
        for lab in model_ordered
    ]
    ...
    for model in model_columns:
        ...
        for ds in dataset_fig3:
            if ds in dataset_list:
                ...
                assert len(values) == 1, f"{ds} should have exactly one mean value."
                mean_value = values[0]
                results[current_metric[0]][model_name][ds] = {"mean": mean_value}
...
if __name__ == "__main__":
    ...
    results, result_info, result_dataset_fig3, result_dataset_fig4 = load_result_fig3_4(
        file_path
    )
    if not len(result_dataset_fig3) == 0:
        for m in result_info:
            plot_scatter_subaxes(
                result_dataset_fig3,
                results[m],
                result_info[m],
                (5.5, 1.8),

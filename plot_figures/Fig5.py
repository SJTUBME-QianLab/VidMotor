...
y_label_dict = {
    "Arising from Chair": "PDMC-Arising-from-Chair\n(5-class)",
...
def subplots_box_vertical(data_dict, figsize=(10, 6), savepath=None):
    ...
    for ax_id, metric in enumerate(metric_list):
        ...
        for act in tasks_copy:
            ...
            for model in model_ordered:
                if model not in mean_values:
                    mean_values[model] = []
                x = np.array(data_dict[metric][act][model])
                mean_values[model].append(np.mean(x))
                pos = i + y_pos_interval
                bplot = ax.boxplot(
                    x,
                    positions=[pos],
                    patch_artist=True,
                ...
                for patch in bplot["boxes"]:
                    patch.set_facecolor(color_models_alpha[model])
        ...
        if pval < 0.0001:
            ax.text(
                (compara_mean_val + our_mean_val) / 2,
                y_line - y_pos_interval * 0.3,
                rf"$P$ < 0.0001",
                ha="center",
                va="top",
...
def plot_std_bars(plot_data, figsize=(3, 6), save_path=None):
    ...
    for ds in tasks:
        mean_vals.append(
            np.array(
                [plot_data[model][ds] for model in model_ordered if model != "Ours"]
            ).mean()
        )
...
def plot_legend(save_path):
    handles = [
        Patch(facecolor=color, label=label) for label, color in color_models.items()
    ]
    plt.figure(figsize=(6.5, 0.2))
    plt.legend(
        handles=handles,
        loc="center",
        ncol=len(color_models),
        frameon=False,
        prop=arial_font,
        handlelength=1.5,
        handletextpad=0.5,
        ...
        for model in model_ordered:
            ...
            for metric in metric_list:
                ...
                for center in center_list:
                    results[task][model][metric][center] = row.iloc[0][
                        f"{metric}_{center}"
                    ]
    plot_std = {}
    for model in model_ordered:
    ...
    for metric in metric_list:
        ...
        for task in tasks:
            ...
            for model in model_ordered:
                ...
                for center in center_list:
                    box_plot_data[metric][task][model].append(
                        results[task][model][metric][center]
                    )
